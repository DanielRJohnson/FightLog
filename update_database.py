...
def scrape_videos(channel_id: str, title_regex: str) -> list[dict]:
    ...
    with yt_dlp.YoutubeDL(opts) as ydl:
        uploads_playlist_id = channel_id.replace("UC", "UU") # UC is channel, UU is uploads
...
if __name__ == '__main__':
    search_list = read_csv("search_list.csv") # list of channels to search, what to search for, etc.

    with sqlite3.connect("matches.db") as conn:
        def process_search_partial(r): # multiprocessing pool likes only one argument
            process_search(r, conn)

        # for each search, update the database 
        n_workers = min(len(search_list), len(os.sched_getaffinity(0))) # max procs needed
        with Pool(n_workers) as pool:
            pool.map(process_search_partial, [row for _, row in search_list.iterrows()])