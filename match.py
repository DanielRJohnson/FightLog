...
class Match:
    ...
    def _extract_names_and_chars_from_chapter(chapter: dict) -> tuple[str, str, str, str]:
        ...
        p1_split, p2_split = split_player(p1), split_player(p2)
        if len(p1_split) == 1:  # edge-case
            p1_split = [p1_split[0], "Not Provided"]
        if len(p2_split) == 1:
            p2_split = [p2_split[0], "Not Provided"]
        
        p1_name, p1_char = remove_whitespace_and_lower_all(p1_split)
        p1_char = take_first_char(p1_char)
        p2_name, p2_char = remove_whitespace_and_lower_all(p2_split)
